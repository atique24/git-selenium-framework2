...
class SeleniumBase:
    ...
    def setEnv(cls, env):
        ...
        elif env.lower() == "batsystest":
            cls.env = "http://batsystest-"

        elif env.lower() == "preprod":
            cls.env = "http://preprod-"

        elif env.lower() == "systest2":
            cls.env = "http://three2:thr33QA@systest2-bat-"

        elif env.lower() == "neptune":
            cls.env = "http://brt1-"
    ...
    def elementClick(self, locator, timeout=30 ,element=None, force=None):
        try:
            if locator:
                element = self.findElement(locator, timeout=timeout)
    ...
    def elementSend(self, locator, message, timeout=30 ,element=None):
        try:
            if locator:
                element = self.findElement(locator, timeout=timeout)
            if element:
    ...
    def getElementsText(self, locator,timeout=30,elements=None):
        ...
        try:
            if locator:
                elements = self.findElements(locator, timeout=timeout)
            if len(elements) > 0:
    ...
    def getAttribute(self, locator, attributeType, element=None, timeout=30):
    ...
    def check_text_on_page(self, text, timeout=30):
        ...
        self.cl.info(value)
        try:
            element = WebDriverWait(self.driver,timeout=timeout).until(ec.visibility_of_element_located(value))
            self.cl.info("Element :: " + element.id + " found for text" + text)
            self.cl.info("Text ::  " + text + " is displayed on the page")
            result = True

        except:
            self.cl.info("Text ::  " + text + " is not displayed on the page")
            raise NoSuchElementException

        return result


    def wait_and_switch_Iframe(self, locator=None, index=None, timeout=30, poll=0.2):
        try:
            ...
            if (locator == None and index==None) or (locator and index):
                raise ValueError("Locator or Index is required.")
    ...
    def getInnerText(self, locator, element=None, timeout=30):
        innerText = None
        try:
            if locator:
                element = self.findElement(locator, timeout=timeout)
    ...
    def doubleClick(self, locator, element=None, timeout=30):
        try:
            if locator:
                element = self.findElement(locator, timeout=timeout)
            if element:
    ...
    def simulate_browser_back(self):
        self.driver.back()
    ...
    def rightClick(self, locator, element=None, timeout=30):
